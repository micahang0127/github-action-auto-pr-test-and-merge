...
5. /pr-create                    → diff 기반 PR 초안 작성 + 사람 승인 후 GitHub PR 생성
```
...
- **명령어**: `/pr-create` 슬래시 커맨드 사용 — AI가 diff 기반으로 템플릿을 채우고 사람 승인 후 생성
- **생성 주체**: 사람이 명시적으로 요청(승인)할 때만 생성 — AI 자동 PR 올리기 금지
- **생성 전 확인**: 현재 브랜치가 `feature/**` 인지 `/pr-create`가 자동 검증
